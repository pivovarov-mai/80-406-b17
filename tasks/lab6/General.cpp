...
Task getTaskInfo(const std::vector<std::string> &system) {
    ...
    idx = system[0].find('=');
    size = system[0].size();
    task.trees.push_back(std::move(FunctionalTree(system[0].substr(idx + 1, size - idx), {"uxx", "ux", "u", "x", "t"})));
    ...
    tmpTree.reset(system[0].substr(0, idx - 1), {"utt", "ut"});
    task.coeff[3] = tmpTree.getCoeff(1).func(0);
