...
class Application(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        label = tk.Label(self, text = "Лабораторная работа №1", font = LARGE_FONT)
        ...
        label5 = tk.Label(self, text = 'Выберете аппроксимацию граничных условий')
        label5.pack()
        ...
        self.button = ttk.Button(self, text = "Построить!", command = self.GetParams)
    ...
    def ErrorWindow(self, note):
        err_win = Tk()
        err_note = tk.Label(err_win, text = note, width = 50, height = 10)
        err_win.title('ERROR')
    ...
    def GetParams(self):
        ...
        if param_a < 0:
            ...
            note = 'Ошибка!\nМомент времени должен быть меньше "t"!'
            self.ErrorWindow(note)
            return
        elif param_a**2 * time_step / space_step**2 > 0.5 and method == 'Явный':
        ...
        if method == 'Явный':
            lab1.explicit(param_t, param_a, space_step, time_step, m, n, aprox, ans_t)
        elif method == "Неявный":
            lab1.implicit(param_t, param_a, space_step, time_step, m, n, aprox, ans_t)
        elif method == "Явный-Неявный":
            lab1.KN(param_t, param_a, space_step, time_step, m, n, aprox, ans_t)

app = Application()
app.title('Лабораторная №1. (c) Алексеев В.Е.')
app.mainloop()