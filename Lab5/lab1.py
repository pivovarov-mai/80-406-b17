...
def explicit(param_t, param_a, space_step, time_step, m, n, aprox, ans_time):
    ...
    for k in range(1, n):
        for j in range(1, m - 1):
            Uarray[k][j] = sigma * (Uarray[k - 1][j + 1] + Uarray[k - 1][j - 1]) + \
                           (1 - 2 * sigma) * Uarray[k - 1][j]
        if aprox == 1:
            Uarray[k][0] = Uarray[k][1] - space_step * Ux1(tmp_time, param_a)
            Uarray[k][m - 1] = Uarray[k][m - 2] + space_step * Ux2(tmp_time, param_a)
        elif aprox == 2:
            Uarray[k][0] = (Ux1(tmp_time, param_a) + Uarray[k][2] / (2 * space_step) -
                            2 * Uarray[k][1] / space_step) * ((2 * space_step) / -3)
            Uarray[k][m - 1] = (Ux2(tmp_time, param_a) - Uarray[k][m - 3] / (2 * space_step) +
                                2 * Uarray[k][m - 2] / space_step) * ((2 * space_step) / 3)
        elif aprox == 3:
            Uarray[k][0] = (Uarray[k][1] - space_step * Ux1(tmp_time, param_a) +
                            (space_step**2 / 2 / time_step) * Uarray[k - 1][0]) / \
                            (1 + space_step**2 / 2 / time_step)
            Uarray[k][m - 1] = (Uarray[k][m - 2] + space_step * Ux2(tmp_time, param_a) +
                                (space_step**2 / 2 / time_step) * Uarray[k - 1][m - 1]) / \
                                (1 + space_step**2 / 2 / time_step)
    ...
    plt.subplot(211)
    plt.gcf().canvas.set_window_title('Лабораторная работа №1')
